...
public:
    ...
    deque<std_msgs::msg::Float64MultiArray> loopInfoVec; // 外部给定的回环对，每个元素是一个数组，每个数组两个元素，0：当前帧，1：历史帧
    ...
    backMapping(rclcpp::Node::SharedPtr node) : ParamLoader(node){
        // isam初始化参数
        ...
        pubKeyPoses = node->create_publisher<sensor_msgs::msg::PointCloud2>("rolo/mapping/trajectory", 1); // 全局路径
        pubLaserCloudSurround = node->create_publisher<sensor_msgs::msg::PointCloud2>("rolo/mapping/map_global", 1); // 全局地图
        ...
        // srvSaveMap  = node->create_service<rolo::SaveMap>("rolo/save_map", std::bind(&backMapping::saveMapService, this, std::placeholders::_1, std::placeholders::_2));

        // 历史关键帧点云
        pubHistoryKeyFrames = node->create_publisher<sensor_msgs::msg::PointCloud2>("rolo/mapping/icp_loop_closure_history_cloud", 1);
        // 关联关键帧点云
        pubIcpKeyFrames = node->create_publisher<sensor_msgs::msg::PointCloud2>("rolo/mapping/icp_loop_closure_corrected_cloud", 1);
        pubLoopConstraintEdge = node->create_publisher<visualization_msgs::msg::MarkerArray>("/rolo/mapping/loop_closure_constraints", 1);
        //  局部地图
        pubRecentKeyFrames = node->create_publisher<sensor_msgs::msg::PointCloud2>("rolo/mapping/map_local", 1);
        // 当前关键帧
        pubRecentKeyFrame = node->create_publisher<sensor_msgs::msg::PointCloud2>("rolo/mapping/cloud_registered", 1);
        pubCloudRegisteredRaw = node->create_publisher<sensor_msgs::msg::PointCloud2>("rolo/mapping/cloud_registered_raw", 1);

        pubSLAMInfo = node->create_publisher<rolo_ros2_interfaces::msg::CloudInfoStamp>("rolo/mapping/slam_info", 1);
    ...
    void updateInitialGuess(){
        ...
        if (cloudInfo.odom_available == true)    // 如果IMU预积分得到了初始里程计估计
    ...
    void publishOdometry()
    {
        ...
        tf2::Quaternion q0;
        q0.setRPY(transformTobeMapped[0], transformTobeMapped[1], transformTobeMapped[2]);
        laserOdometryROS.pose.pose.orientation.x = q0.x();
        laserOdometryROS.pose.pose.orientation.y = q0.y();
        laserOdometryROS.pose.pose.orientation.z = q0.z();
        laserOdometryROS.pose.pose.orientation.w = q0.w();
        ...
        t_odom_to_lidar.setRotation(q1);
        t_odom_to_lidar.setOrigin(tf2::Vector3(transformTobeMapped[3], transformTobeMapped[4], transformTobeMapped[5]));

        // 转换为 geometry_msgs::msg::TransformStamped
        ...
        trans_odom_to_lidar.transform.translation.x = t_odom_to_lidar.getBasis()[0][3];
        trans_odom_to_lidar.transform.translation.y = t_odom_to_lidar.getBasis()[1][3];
        trans_odom_to_lidar.transform.translation.z = t_odom_to_lidar.getBasis()[2][3];

        trans_odom_to_lidar.transform.rotation.x = q1.x();
        ...
        if (lastIncreOdomPubFlag == false)
        {
            ...
            laserOdomIncremental.pose.pose.orientation.x = q.x();
            laserOdomIncremental.pose.pose.orientation.y = q.y();
            laserOdomIncremental.pose.pose.orientation.z = q.z();
            laserOdomIncremental.pose.pose.orientation.w = q.w();
    ...
    void saveTUM(){
        ...
        for (int i=0 ; i<cloudKeyPoses6D->size(); i++){
            tf2::Quaternion qu;
            qu.setRPY(cloudKeyPoses6D->points[i].roll, cloudKeyPoses6D->points[i].pitch, cloudKeyPoses6D->points[i].yaw);
