...
public:
    ...
    rclcpp::Subscription<rolo_ros2_interfaces::msg::CloudInfoStamp>::SharedPtr subLaserCloudInfo;
    ...
    FeatureExtraction(rclcpp::Node::SharedPtr node) : ParamLoader(node){

        // //! 输入：image_projection传来的cloud_info，输出：角点点云，平面点点云，feature_cloud_info
        subLaserCloudInfo = node->create_subscription<rolo_ros2_interfaces::msg::CloudInfoStamp>("rolo/cloud_info", 1, std::bind(&FeatureExtraction::laserCloudInfoHandler, this, std::placeholders::_1));
